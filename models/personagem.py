...
class Aventureiro:
    ...
    def usar_habilidade(
        self, nome: str, alvo: Optional["Aventureiro"] = None, **kwargs
    ):
...
class Guerreiro(Aventureiro):
    ...
    def receber_dano(self, dano: float) -> int:
        ...
        if (
            self._vida <= 0
            and self._vontade_de_ferro_ativa
            and "vontade_de_ferro" in self._habilidades
        ):
...
class Mago(Aventureiro):
    ...
    def receber_dano(self, dano: float) -> int:
        if (
            self._summon
            and self._summon.tipo == "elefante"
            and self._summon.hp
            and self._summon.hp > 0
        ):
